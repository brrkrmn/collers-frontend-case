...
const JoinSection = () => {
  return (
    <div className="w-full h-full laptop:px-20 px-4 py-8 laptop:py-40 flex items-center justify-center overflow-hidden">
      <div className="w-full rounded-xl bg-background-light overflow-visible px-4 pb-40 laptop:p-20 shadow-sm flex flex-col laptop:flex-row items-center justify-center">
        ...
        <div className="laptop:w-full w-[60%] h-full relative flex items-center justify-center overflow-visible">
          <DesktopScreen image={Video} className="absolute z-10 aspect-[6/4] scale-[0.95] -right-4 -top-[162px]" />
          <Image src={Background} alt="Background" className="absolute scale-[1.3] -right-10 -top-56 drop-shadow-xl" />
          <Image src={Circle} alt="Circle" className="absolute z-20 -bottom-60 left-24" />
        </div>
      </div>
    </div>
  )
